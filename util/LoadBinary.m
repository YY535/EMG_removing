...
%         handle single type
%   allows to load data from within certain time epochs , give
...
%
%  Loading methods (1-5) differ by the way the loading is done - depending on case for
% efficiency purposes some are better then others, default is 2;
%
% METHODS:
% 1: loads the whole file (not working with Periods-specific loading).
%    benefits from presence of continuous patches in list of loaded channels, which uses skips in fread to do efficient loading.
%
% 2: works using fread going through buffers. This works even for huge files under low memory conditions if only subset of channels are needed.
%
% 3: loads one by one Periods, works best if they are small and many.
% 4: uses memory mapping to provide fast (if you have enough RAM) and easy way to load subset of data.
% NB!!! for efficiency and historical reasons output is nCh x nT, opposite from other methods. Watch out!
% 5: this one attempts to pass memmapfile object of the whole file out of the function (2nd output, instead of OrigIndex)
%    to allow fast repeated loading from the same file in consequitive calls to LoadBinary. Then FileName will be relaced by memmapfile obect.
% Resample specifices amount of subsampling. at the moment no antialyasing
...
if ~isobject(FileName)
    ...
    if ~FileExists(FileName)
         FileName = ResolvePath(FileName,0);
    end
    if ~FileExists(FileName) 
        if FileExists([FileName(1:end-3) 'eeg'])
            FileName = [FileName(1:end-3) 'eeg'];
        elseif FileExists([FileName(1:end-3) 'lfp'])
            FileName = [FileName(1:end-3) 'lfp'];
        else
            error('File with this name couldnt be found');
        end
    end
...
if Resample>1
    warning('Resampling is implemented in a dumb fast way and doesn''t include antialiasing!!! Beware or fix! :)');
end

%matver = version; matver = str2num(matver(1));
%if matver<7 outtype ='double';end

PrecString =[intype '=>' outtype];
if isobject(FileName)
    method = 5;
    nSamples = FileName.Format{2}(2);
else
...
if method==1 & ~isempty(Periods)
    error('this method does not perform (yet) selective loading with Periods, use method 3 or 4 )');
end
...
switch method
    ...
    case 4 %new way - with memmapfile
        ...
        if isempty(Periods)
            ...
            for ii=1:nPeriods
                ...
                %                data = [data mmap.Data.x(Channels,Periods(ii,1):Resample:Periods(ii,2))];
    ...
    case 5 %with repeated calls using memmapfile as a return;
        if ~isobject(FileName)
        ...
        if isempty(Periods)
            ...
            Ind = logical(WithinRanges([1:Resample:mmap.Format{2}(2)],Periods));
            data = mmap.Data.x(Channels,Ind);
        end
        data = cast(data,outtype);
