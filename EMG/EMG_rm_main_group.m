...
% EMG_rm_main_group(FileBases,denoise_groups,[denoise_frequency_lowerbound, 
%                        keep_former, savedir,...
%                        cleanEMGch,...
...
%                  {[shk_1,shank2],shk_n} 
%   Optional:
%       denoise_frequency_lowerbound: keep slower signal until this frequency, default: 0
...
% 
[denoise_frequency_lowerbound,keep_former,savedir,cleanEMGch,silence_periods,sp_loadingfuns,rm_linenoise,line_thrd, numOfIC, hp_freq,nchunks, cmp_method,down_sample,nFFT, Winlength, save_together,use_wb,PeriodLengthLimits] = ...
    DefaultArgs(varargin, {0,false,pwd,[],false,[],true,1.8,[], 100,6, 'hw',3, 2^9,[],true,false,1e6});
...
for n = 1:nG
    ...
    for k = 1:nPeriod
        ...
        [~, Ws{k,n}, As{k,n}, EMG_au{k,n}, AW{k,n}, armodel,scaling_factor(k,n),flatness{k,n}] = EMG_rm_long(LoadBinary(LFPfile,HP,par.nChannels,2,[],[],tmp_Period)',   ...
            ...
            true, save_range, FileBase, savedir, save_together,use_wb,denoise_groups{n},HP,Shk01{n},EMGFileName);
    end
end
par.denoise_frequency_lowerbound =denoise_frequency_lowerbound;
par.keep_former =keep_former;
par.savedir =savedir;
...
if save_together
    ...
    save(sprintf('%s/%s.EMG_rm.G%s.mat',savedir,FileBase,shank_names(1:(end-1))), 'Ws','As','AW','EMG_au','armodel','sug_period','par','denoise_shank','LFPfile','scaling_factor','G_par','flatness')
end
